...
/*   Updated: 2021/04/24 14:34:10 by cgutierr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/cub_bonus.h"

static void	sprite_bucle(t_cub *cub, int *y, int *d, t_sprite *spr)
{
	cub->rc_spr.texx = (int)(256 * (cub->rc_spr.stripe
				- (-cub->rc_spr.spriteWidth / 2 + cub->rc_spr.spr_screenx))
			* cub->txtrs.width[spr->texture] / cub->rc_spr.spriteWidth) / 256;
		...
		*y = cub->rc_spr.drawstarty;
		while (*y < cub->rc_spr.drawendy)
		{
			*d = (*y) * 256 - cub->screen.y
			...
			cub->rc_spr.texy = ((*d * cub->txtrs.height[spr->texture])
					/ cub->rc_spr.spr_height) / 256;
			cub->data.color = my_pixel_get(&cub->txtrs.img[spr->texture],
...
static void	sprite_set(t_sprite **sprite_actual)
{
	(*sprite_actual)->delay += 1;
	if ((*sprite_actual)->texture == 9 && (*sprite_actual)->delay >= 25)
	{
		(*sprite_actual)->texture = 11;
		(*sprite_actual)->delay = 0;
	}
	else if ((*sprite_actual)->texture == 11 && (*sprite_actual)->delay >= 25)
	{
		(*sprite_actual)->texture = 9;
		(*sprite_actual)->delay = 0;
	}
}
...
void	sprites(t_cub *cub)
{
	...
	while (i < cub->txtrs.num_sprites)
	{
		...
		{
			sprite_bucle(cub, &y, &d, sprite_actual);
			cub->rc_spr.stripe++;
		}
		sopita = sopita->next;
		sprite_set(&sprite_actual);
		i++;
	}
}
