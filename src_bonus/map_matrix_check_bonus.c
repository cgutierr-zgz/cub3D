...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   map_matrix_check_bonus.c                           :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: cgutierr <cgutierr@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/03/26 00:48:54 by cgutierr          #+#    #+#             */
/*   Updated: 2021/04/26 18:55:05 by cgutierr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	check_aux(t_cub *cub, int j, int j2, int x)
{
	if (x)
	{
		if (j2 == 0 && ft_strchr("023459", cub->map.map_matrix[j][j2]))
			map_ko_matrix(cub, j, j2);
		if (j == cub->map.map_lines - 1
			&& ft_strchr("023459", cub->map.map_matrix[j][j2]))
			map_ko_matrix(cub, j, j2);
	}
	else
	{
		if ((cub->map.map_matrix[j][j2] == ' '
			...
			|| (cub->map.map_matrix[j][j2] == '3')
			|| (cub->map.map_matrix[j][j2] == '4')
			|| (cub->map.map_matrix[j][j2] == '5')
			|| (cub->map.map_matrix[j][j2] == '9'))
			map_ko_matrix(cub, j, j2);
	}
}
...
static void	bucle(int j, int j2, t_cub *cub)
{
	...
	if (j2 == cub->map.max_len - 1)
		...
		&& (ft_strchr("023459", cub->map.map_matrix[j][j2 + 1]))
		&& (cub->map.map_matrix[j - 1][j2] == ' '))
		|| ((cub->map.map_matrix[j][j2] == '1')
		&& (ft_strchr("023459", cub->map.map_matrix[j - 1][j2]))
		&& (cub->map.map_matrix[j][j2 + 1] == ' ')));
