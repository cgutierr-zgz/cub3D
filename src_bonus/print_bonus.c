...
/*   Updated: 2021/04/26 17:06:28 by cgutierr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	print_map_info(t_cub *cub)
{
	...
	if (cub->start_tp.x && cub->start_tp.y && cub->end_tp.x && cub->end_tp.y)
	{
		printf("Teleport points = "RESET);
		printf(YELLOW"%d,%d <-> %d,%d\n"RESET,
			cub->start_tp.x, cub->start_tp.y, cub->end_tp.x, cub->end_tp.y);
	}
	print_map(cub, 0);
}
